...
class PineLabsController(Controller):
    ...
    def pinelabs_webhook(self):
        ...
        _logger.info("Full request headers: %s", pprint.pformat(dict(request.httprequest.headers)))
        _logger.info("Full request body: %s", request.httprequest.get_data())
        ...
        if event_type in pine_labs_const.HANDLED_WEBHOOK_EVENTS:
            ...
            # Remove version prefix dynamically
        if received_signature and "," in received_signature:
            received_signature = received_signature.split(",", 1)[1]
            tx_sudo = request.env['payment.transaction'].sudo()._search_by_reference(
                'pine_labs', entity_data
            )
            signed_content = f"{request.httprequest.headers.get('Webhook-Id')}.{request.httprequest.headers.get('Webhook-Timestamp')}.{raw_body}"
            if tx_sudo:
    ...
    def _verify_signature(
        ...
        if (
            expected_signature is None or not hmac.compare_digest(received_signature, expected_signature)
        ):
